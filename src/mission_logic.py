...
class MissionLogic:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Mission parameters
        self.max_mission_duration = config.get(
    ...
    def start_mission(self, patrol_waypoints: List[tuple]):
        ...
        self.logger.info(
            f"Mission started with {len(patrol_waypoints)} patrol waypoints"
        )
                ...
                if not self.target_detected:
                    ...
                    self.target_info = TargetInfo(
                        position=(
                            ...
                            0,
                            0,
                        ),  # Would need to convert from image to world coordinates
                        confidence=obj.get("confidence", 0),
                        timestamp=time.time(),
                        bbox=obj.get("bbox"),
                    )
                    self.logger.info(
                        f"Target detected with confidence {obj.get('confidence', 0)}"
                    )
                break
        else:
    ...
    def _handle_return_home_state(self) -> Dict[str, Any]:
        ...
        return {"action": "land", "reason": "mission_completed"}
    ...
    def _handle_obstacle_avoidance(
        self, vision_results: Dict[str, Any]
    ) -> Dict[str, Any]:
